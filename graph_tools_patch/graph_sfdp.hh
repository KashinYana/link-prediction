...
template <class Pos>
inline double get_diff(const Pos& p1, const Pos& p2, Pos& r, double d=0)
{
    double abs = 0;
    for (size_t i = 0; i < 2; ++i)
    {
                 ...
                 [&](size_t, auto v)
                 {
                     ...
                     while (!Q.empty())
                     {
                         ...
                         if (!dleafs.empty())
                         {
                             ...
                             {
                                 val_t d = get_diff(get<0>(dleaf), pos[v], diff, d);
                                 if (d == 0)
                                     continue;
                         ...
                         else
                         {
                             double w = q.get_w();
                             q.get_cm(cm);
                             double d = get_diff(cm, pos[v], diff, d);
                             if (w > theta * d)
                             {
                     ...
                     if (gamma > 0)
                     {
                         for (size_t s = 0; s < group_cm.size(); ++s)
                         {
                             ...
                             if (s == size_t(group[v]))
                                 continue;
                             val_t d = get_diff(group_cm[s], pos[v], diff, d);
                             if (d == 0)
                                 continue;
                     ...
                     {
                         val_t d = get_diff(group_cm[group[v]], pos[v], diff, d);
                         if (d > 0)
                         {
